...
<!--  
  ...
  ----------------------------------------------------------------------------------------------------------------------------
    ...
    속성 상속
      ...
      여기서 <MyButton>은 class를 허용되는 프로퍼티로 선언하지 않았다. 따라서 class는 대체 속성으로 처리되어
      <MyButton>의 루트 엘리먼트에 자동으로 추가된다.

    class와 style의 병합

      자식 컴포넌트의 루트 엘리먼트에 이미 class 또는 style 속성이 있는 경우,
      상위 엘리먼트에서 상속된 class 또는 style 값과 병합된다. 이전 예에서 <MyButton>의 템플릿을 다음과 같이 변경한다면
    ...
    v-on 리스너 상속
      ...
      <button>에 이미 v-on으로 바인딩된 click 리스너가 있는 경우 두 리스너가 모두 트리거 된다.

    
    중첩된 컴포넌트 상속

      컴포넌트가 다른 컴포넌트를 루트 노드로 렌더링하는 경우, 예를 들어 <MyButton>을 리펙터링 하여
      <BaseButton>을 루트로 렌더링한다.

        //다른 컴포넌트 하나를 렌더링하는 <MyButton /> 템플릿
        <BaseButton />

      그러면 <MyButton>이 수신한 폴스루 속성이 자동으로 <BaseButton>으로 전달된다.
  ...
  ----------------------------------------------------------------------------------------------------------------------------
    ...
    속성 상속 비활성화
      ...
      inheritAttrs 옵션을 false로 설정하면 폴스루 속성을 적용해야 하는 위치를 완전히 제어할 수 있다.

      이러한 폴스루 속성은 템플릿 표현식에서 $attrs 로 직접 접근할 수 있다.

        <span>폴스루 속성: {{ $attrs }}</span>

      $attrs 객체에는 컴포넌트의 props 또는 emits 옵션으로 선언되지 않은 모든 속성(예: class, style, v-on 리스너 등)이 포함된다.

      참고
        - props와 달리 폴스루 속성은 JavaScript에서 원 표기를 유지하므로 foo-bar와 같은 속성은 $attrs['foo-bar']로 접근해야 한다.
        - @click 과 같은 v-on 이벤트 리스너는 $attrs.onClick 속성에 노출 된다.
      ...
      여기서 class 및 v-on 리스너와 같은 폴스루 속싱이 외부 <div>가 아닌 내부 <button>에 적용되기를 원할 수 있다.
      ...
      인자없는 v-bind는 개체의 모든 속성을 대상 엘리먼트의 속성으로 묶는다는 것을 기억하자.

  ----------------------------------------------------------------------------------------------------------------------------

    다중 루트 노드에서 속성 상속

      단일 루트 노드가 있는 컴포넌트와 달리 여러 루트 노드가 있는 컴포넌트에서 자동 속성 폴스루 동작이 없다.
      $attrs가 명시적으로 바인딩되지 않은 경우 런타임 경고가 발생된다.
      ...
      $attrs가 명시적으로 바인딩된 경우, 경고가 표시되지 않는다.
  ...
  ----------------------------------------------------------------------------------------------------------------------------
    ...
    JavaScript에서 폴스루 속성 접근하기
      ...
      필요한 경우 useAttrs() API를 사용하여 <script setup>에서 컴포넌트의 폴스루 속성에 접근할 수 있다.
      ...
      <script setup>을 사용하지 않으면 attrs가 setup() 컨텍스트의 속성으로 노출된다.
        ...
        export default {
          setup(props, ctx) {
            // 폴스루 속성은 ctx.attrs로 노출된다.
            console.log(ctx.attrs)
          }
        }

      여기서 attrs 객체는 항상 최신 폴스루 속성을 반영하지만 (성능상의 이유로) 반응하지 않는다.
      감시자를 사용하여 변경 사항을 관찰할 수 없다. 반응성이 필요하다면 prop을 사용해야 한다.
