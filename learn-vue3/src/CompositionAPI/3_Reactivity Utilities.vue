...
<!--  
  ...
  isProxy() 
    ...
    객체가 reactive(), readonly() shallowReactive() 또는 shallowReadonly()에 의해 생성된 프록시인지 확인한다.
  ...
  객체가 reactive() 또는 shallowReactive()에 의해 생성된 프록시인지 확인한다.
  ...
  isReadonly()
    ...
    전달된 값이 읽기 전용 객체인지 확인한다.
    읽기 전용 객체의 속성은 변경할 수 있지만 전달된 객체를 통해 직접 할당할 수는 없다.

    readonly() 및 shallowReadonly()로 생성된 프록시는 모두 읽기 전용으로 간주되며, set 함수가 없는 computed() 참조와 마찬가지다.
