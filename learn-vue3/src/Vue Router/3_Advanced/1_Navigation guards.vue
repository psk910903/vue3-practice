...
<!--  
  ...
  복습
    ...
    네비게이션 가드는 라우터의 전환을 제어하고 특정 조건에 따라 네비게이션을 허용하거나 중단시키는 기능을 제공한다.
    이를 통해 라우터의 전환에 대한 세밀한 제어를 할 수 있다.

    Vue Router는 네비게이션 가드로 다음과 같은 다양한 기능을 제공한다.

    1.전역 네비게이션 가드(Global Navigation Guards)
      - beforeEach: 라우터의 각 전환 전에 실행되는 전역 가드로, 모든 라우터에 적용된다. 이 가드에서 특정 조건을 검사하여 네비게이션을 중단하거나 허용할 수 있다.
      - beforeResolve: 네비게이션이 완료되기 전에 실행되는 전역 가드이다. 라우터의 각 전환 이후에 호출된다.

    2.포인트 네비게이션 가드(Point Navigation Guards)
      - beforeEnter: 특정 라우트로의 전환 직전에 실행되는 라우트별 가드이다. 해당 라우트의 beforeEnter 속성에 함수를 설정하여 사용한다.
      - beforeRouteUpdate: 현재 라우트에서 다른 라우트로 전환할 때 호출되는 라우트별 가드이다. 동일한 컴포넌트 인스턴스를 재사용하는 경우에 호출된다.
      - beforeRouteLeave: 현재 라우트에서 다른 라우트로 전환하기 전에 호출되는 라우트별 가드이다. 사용자가 페이지를 떠나기 전에 확인 대화상자를 표시하거나 변경 사항을 저장하는 등의 작업을 수행할수 있다.

    3.컴포넌트 내부의 가드(Inside Component Guards)
      - beforeRouteEnter: 라우트로 이동할 때 해당 컴포넌트가 아직 생성되지 않았을 때 호출되는 가드이다. 따라서 이 가드 내에서는 컴포넌트 인스턴스에 직접 접근할 수 없다.
      - beforeRouteUpdate: 컴포넌트가 이미 생성되어 있는 경우에만 호출되는 가드로, 동일한 컴포넌트 인스턴스를 재사용하는 경우에 사용된다.
      - beforeRouteLeave: 컴포넌트를 떠나기 전에 호출되는 가드로, 컴포넌트 내부에서 라우트 전환을 중단하거나 확인 대화상자를 표시하는 등의 작업을 수행할 수 있다.

    이러한 네비게이션 가드를 사용하여 라우터의 전환을 관리하고 사용자 경험을 개선할 수 있다.
    예를 들어, 사용자 인증 여부를 확인하여 인증되지 않은 사용자가 특정 페이지에 접근하는 것을 막을 수 있거나, 페이지를 떠나기 전에 변경 사항을 저장할 것인지 묻는 확인 대화상자를 표시할 수 있다.
