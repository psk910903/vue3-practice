  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    관례와 모범 사례
      ...
      3.반환 값
        ...
        4.사이드 이팩트
          컴포저블에서 사이드 이팩트(예:DOM 이벤트 리스너 추가 또는 데이터 가져오기)를 수행하는 것은 괜찮지만 다음 규칙에 주의해야 한다.

            - 서버 사이드 렌더링(SSR)을 사용하는 앱에서 작업하는 경우,
              마운트 후 생명주기 훅인 onMounted()에서 DOM관련 사이드 이팩트를 수행해야 한다.
              이 훅은 브라어주에서만 호출되므로 내부 코드가 DOM에 접근할 수 있는지 알 수 있다.

            - onUnmounted() 에서 사이드 이팩트를 마무리 지었는지 확인해야한다.
