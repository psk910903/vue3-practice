...
<!--  
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    기본 예제
      ...
      그러나 상태 변경에 대한 반응으로 "사이드 이팩트"(예: DOM을 변경하거나 비동기 작업의 결과를 기반으로 다른 상태를 변경하는 것)을 수행해야 하는 경우가 있다.
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    watchEffect()
      ...
      watch vs watchEffect
        ...
        watch와 'watchEffect' 둘 다 사이드 이팩트를 반응적으로 실행할 수 있게 해준다.
        주요 차이점은 반응형 의존성을 추적하는 방식이다.

          - watch는 명시적으로 감시된 소스만 추적한다. 콜백 내에서 조회하는 항목은 추적하지 않는다.
            또한 콜백은 소스가 실제로 변경된 경우에만 트리거된다. watch는 의존성 추적을 사이드 이팩트와 분리하여,
            콜백이 실행되어야 하는 시기를 보다 정확하게 제어할 수 있다.

          - 반면 watchEffect는 의존성 추적과 사이드 이팩트를 하나의 단계로 결합한다.
  ...
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Vue.js 3d에서 watch와 watchEffect는 상태 변화를 감지하고 특정 동작을 수행하는 데 사용되는 기능이다.

    watch

      watch 함수는 특정 데이터나 계산된 속성 등을 감시하고, 해당 값이 변할 때마다 콜백 함수를 실행한다.
      일반적으로 명시적으로 특정 속성을 감시할 때 사용된다.
      ...
      watch(
        ...
        //값이 변경될 때 실행될 콜백 함수
        (newValue, oldValue) => {
          //변경된 값과 이전 값에 대한 로직 수행
          console.log(`값이 변경되었습니다: ${oldValue} -> ${newValue}`);
        }
      )

      특징
        - 'deep' 옵션을 사용하여 객체나 배열 내부의 값까지 감시할 수 있다.
        - 'immediate' 옵션을 사용하여 감시를 시작할 때 즉시 콜백을 실행할 수 있다.


    watchEffect
    
      watchEffect 함수는 특정 데이터나 계산된 속성을 감시하고, 해당 값이 변경될 때마다 자동으로 실행되는 함수를 정의한다.
      주로 뷰 컴포넌트 내에서 반응형 데이터의 변경에 따라 부수 효과를 처리할 때 사용된다.

      //watchEffect 함수를 사용하는 방법
      watchEffect(() => {
        //의존하는 상태나 계싼된 속성에 대한 로직을 수행
        console.log(`someValue의 값이 변경되었습니다: ${state.someValue}`);
      })

      특징
        - watchEffect는 자동으로 의존하는 데이터를 추적하므로 명시적으로 의존성을 지정할 필요가 없다.
        - 컴포넌트가 렌더링 될 때마다 자동으로 실행된다.

      watch와 watchEffect는 각각의 상황에 맞게 선택하여 사용하면 된다.
      watch는 명시적인 감시가 필요한 경우에 유용하고, watchEffect는 간편하게 부수 효과를 처리할 때 유용하다.
